...
const Post = (props: postProps) => {
    ...
    const { enablePostOfContents, disqusShortname, enableSocialShare }: iConfig = config.post;
                    ...
                    {`
    {
      ...
      "headline": "${title}",
      ${config.bio.profileImageFileName
            ? `"publisher": {
        "@type" : "organization",
        "name" : "${config.name}",
        "logo": {
          "@type": "ImageObject",
          "url": "${config.siteUrl}${require(`../images/${config.bio.profileImageFileName}`)}"
        }
      },
      "image": ["${config.siteUrl}${require(`../images/${config.bio.profileImageFileName}`)}"]`
            : `"publisher": {
        "@type" : "organization",
        "name" : "${config.name}"
      },
                    ...
                    {!config.googleAD_on ? null :
                        ...
                        (isDevelopment ? (
                            ...
                            <>
                                <aside className="ad">
                                    <AdSense.Google
                                        client={config.googleAD.googleAdsenseClient || 'ca-pub-5001380215831339'}
                                        slot={config.googleAD.googleAdsenseSlot || '5214956675'}
                                        style={{ display: 'block' }}
                                        format="auto"
                                        responsive="true"
                                    />
